...
export const generateCustomerBasedQuery = (
    isAdmin,
    brand,
    valueLow,
    valueHigh,
    ...
    const serializeFilters = filters => filters.length > 0 ? ` WHERE ${filters.join(' AND ')}` : '';
    const serializeFields = fields => fields.length > 0 ? fields.join(', ') : '*';
    return `SELECT ${serializeFields(queryFields)} FROM  (SELECT * FROM BuySell${serializeFilters(buySellFilters)}) AS b INNER JOIN Counterparty AS c ON b.CustID = c.CpID INNER JOIN Counterparty AS v ON b.VendorID = v.CpID INNER JOIN (SELECT * FROM WksDetail${serializeFilters(workDetailsFilters)}) AS w ON w.BuySellNo = b.BuySellNo ORDER BY InvoiceNo`;
};
...
export const getIndex = ({
    ...
    dueDateFrom,
    dueDateTo
}) => axios.post(`${config.url_sql}/sql`, { query: generateIndex(isAdmin, page, pageSize, brand, quantityLow, quantityHigh, customer, poNumber, soNumber, destinationCountry, productType, orderDateFrom, orderDateTo, dueDateFrom, dueDateTo) }, { headers: { authorization: `Bearer ${getToken()}` } });
export const getByBrand = ({
    ...
    dueDateFrom,
    dueDateTo
}) => axios.post(`${config.url_sql}/sql`, { query: generateBrandBasedQuery(isAdmin, page, pageSize, brand, valueLow, valueHigh, customer, invoiceNumber, productType, orderDateFrom, orderDateTo, dueDateFrom, dueDateTo) }, { headers: { authorization: `Bearer ${getToken()}` } });
export const getByCustomer = ({
    ...
    dueDateFrom,
    dueDateTo
}) => axios.post(`${config.url_sql}/sql`, { query: generateCustomerBasedQuery(isAdmin, brand, valueLow, valueHigh, customer, invoiceNumber, productType, orderDateFrom, orderDateTo, dueDateFrom, dueDateTo) }, { headers: { authorization: `Bearer ${getToken()}` } });
