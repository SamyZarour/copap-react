...
const generateTraderQuery = () => 'SELECT DISTINCT ReferrerNo as label, ReferrerNo as value FROM Referrer';
...
const generateCustomerLastPurchaseQuery = customer => `SELECT TOP 1 b.InvoiceDt FROM (SELECT CpID FROM CounterParty WHERE CpID='${customer}') AS c INNER JOIN BuySell AS b ON c.CpID = b.CustID`;
...
export const generateInvoicesQuery = (
    ...
    const queryFields = [...(isAdmin ? adminFields : []), { field: 'b.SBalanceDue', label: 'SaleBalanceDue' }, { field: 'd.DeliveryDt', label: 'DeliveryDate' }, { field: 'b.InvoiceDt', label: 'InvoiceDate' }, { field: 'b.ShipFromID', label: 'ShipFromID' }, { field: 'b.ShippingDt', label: 'ShippingDate' }, { field: 'b.CompletedDt', label: 'CompletedDate' }, { field: 'b.BuySellNo', label: 'InvoiceNo' }, { field: 'b.ReferrerNo1', label: 'SalesRep' }, { field: 'b.ReferrerNo2', label: 'BuyRep' }, { field: 'c.CompanyNm', label: 'Client' }, { field: 'b.OrderDt', label: 'OrderDate' }, { field: 'b.TradeType', label: 'ProductType' }, { field: 'v.CompanyNm', label: 'Supplier' }, { field: 'w.SWeightUOM', label: 'QtyUom' }, { field: 'w.SPrice', label: 'PriceSale' }, { field: 'w.SPriceUOM', label: 'PriceUomSale' }, { field: 'w.SCurrencyCd', label: 'CurrencySale' }, { field: 'b.DueDt', label: 'DueDate' }, { field: 'b.AltCoEmail', label: 'ContactPoint' }];
    const buySellFilters = [`(ReferrerNo1 LIKE '${(!isAdmin && userId) || '%'}' OR ReferrerNo2 LIKE '${(!isAdmin && userId) || '%'}')`, `VendorID LIKE '${brand || '%'}'`, `CustID LIKE '${customer || '%'}'`, `BuySellNo LIKE '${invoiceNumber || '%'}'`, `RefPONo LIKE '${poNumber || '%'}'`, `RefSONo LIKE '${soNumber || '%'}'`, `TradeType LIKE '${productType || '%'}'`, `ShipToID LIKE '${destinationCountry || '%'}'`, `InvoiceDt >= '${invoiceDateFrom || '01/01/1970'}'`, `InvoiceDt ${invoiceDateTo ? `<= '${invoiceDateTo}'` : '> \'01/01/1970\''}`, `OrderDt >= '${orderDateFrom || '01/01/1970'}'`, `OrderDt ${orderDateTo ? `<= '${orderDateTo}'` : '> \'01/01/1970\''}`, `DueDt >= '${dueDateFrom || '01/01/1970'}'`, `DueDt ${dueDateTo ? `<= '${dueDateTo}'` : '> \'01/01/1970\''}`, ['0', '1'].includes(invoiceType) ? `Completed = ${invoiceType}` : '(Completed = 1 OR Completed = 0)'];
...
export const getInvoices = ({
    ...
    dueDateFrom,
    dueDateTo
}) => axios.post(`${config.url}/api/sql`, { query: generateInvoicesQuery(isAdmin, isCount, isPaged, userId, invoiceType, page, pageSize, brand, valueLow, valueHigh, quantityLow, quantityHigh, customer, invoiceNumber, poNumber, soNumber, productType, destinationCountry, orderDateFrom, orderDateTo, invoiceDateFrom, invoiceDateTo, dueDateFrom, dueDateTo) }, { headers: { authorization: `Bearer ${getToken()}` } });
